...
til::frame_size_calculator::~frame_size_calculator() {
  os().flush();
}
...
void til::frame_size_calculator::do_object_node(til::object_node *const node, int lvl) {
  // EMPTY
}
...
void til::frame_size_calculator::do_block_node(til::block_node *const node, int lvl) {
  if (node->varDeclarations()) node->varDeclarations()->accept(this, lvl + 2);
  if (node->instructions()) node->instructions()->accept(this, lvl + 2);
}

void til::frame_size_calculator::do_process_node(til::process_node *const node, int lvl) {

}

void til::frame_size_calculator::do_iterate_node(til::iterate_node *const node, int lvl) {
